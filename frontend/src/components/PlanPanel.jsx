...
const PlanPanel = ({ onBack, input, setInput, onMetricsUpdate, activePlan, onCompleteExecution }) => {
  ...
  const initiateProtocol = useCallback(async (missionIdOverride = null) => {
    ...
    if (missionIdOverride) {
      console.log("Setting mission id: ", targetId);
    ...
    try {
      // 1. Fire a structured request to start background execution
      const response = await aiService.executeMission(targetId);
      
      // 2. Safely capture the Celery background worker ID tracking reference
      const data = response.data || response;
      console.log("data recieved: ", data);
      if (data && data.celery_task_id) {
        console.log("Background Worker Scheduled successfully with Task ID:", data.celery_task_id);
        setceleryId(data.celery_task_id);
      } else {
        console.warn("Mission was queued, but no celery_task_id identifier was received.", data);
      }
    } catch (err) {
  ...
  // --- 3. PERSISTENT POLLING LOOP: TRANSITION AGENT STATES AND TIMER DROPS ---
  useEffect(() => {
    if (!celeryId || isPaused || status !== 'executing') return;

    let isMounted = true;

    const pollWorkerTelemetry = async () => {
      try {
        // Query the live background runtime task tracking metrics
        const response = await aiService.getExecutionStatus(celeryId);
        const payload = response.data || response;
        console.log("Payload received: ", payload);
        if (!isMounted) return;
        console.log(`[Worker Sync - ${celeryId}] Status Payload:`, payload);

        // Standardize structural mapping if state data falls into metadata dictionaries
        const taskState = payload.state;
        const eventData = payload.data || {};

        if (eventData && eventData.event) {
          console.log("Processing Worker Event Matrix:", eventData.event);
          
          switch (eventData.event) {
            case "MANIFEST":
              if (eventData.steps && eventData.steps.length > 0) {
                setSteps(eventData.steps.map(s => ({
            ...
            case "STRATEGIC_INTERRUPT":
              setIsPaused(true);
              if (eventData.index !== undefined) setCurrentStep(eventData.index);
              setApprovalData({
              ...
              setEditableArtifact(`[CLARIFICATION REQUESTED]: ${eventData.reason}\n\nMy response: `);
              break;
            
            case "STEP_STARTED":
              const idx = eventData.index ?? 0;
              console.log("MOVING_TO_STEP:", idx);
              setCurrentStep(idx);
              
              setSteps(prev => prev.map((s, i) => 
                i === idx ? {
                  ...s,
                  tool_required: eventData.steps?.[idx]?.tool_required || s.tool_required,
                  logic_reasoning: eventData.steps?.[idx]?.logic_reasoning || s.logic_reasoning
                } : s
              ));

              const duration = eventData.steps?.[idx]?.time_allocated || steps[idx]?.time_allocated || 60;
            ...
            case "REQUIRE_APPROVAL":
              setIsPaused(true);
              setApprovalData({
                step_id: eventData.step_id || eventData.backend_step_id, 
                index: eventData.index,
                artifact: eventData.content?.description || "",
                estimated: steps[eventData.index]?.time_allocated || 0,
                actual: eventData.content?.time_needed || 0,
                drift: eventData.content?.drift || 0
              });
              setEditableArtifact(eventData.content?.description || "");
              break;

            case "STEP_COMPLETED":
              console.log(`Step ${eventData.index} verified completely.`);
              setSteps(prev => prev.map((s, i) => i === eventData.index ? { ...s, status: 'completed' } : s));
              setApprovalData(null);
              break;
            ...
            case "MISSION_COMPLETED":
              ...
              setSteps([]);
              if (typeof onCompleteExecution === 'function') {
                onCompleteExecution();
              } else {
            ...
            case "TELEMETRY_PULSE":
              if (eventData.metrics) {
                setLiveMetrics(eventData.metrics);
                onMetricsUpdate({
                  latency: eventData.metrics.step_latency || 0,
                  interrupts: eventData.metrics.interrupt_count || 0,
                  progress: eventData.metrics.progress || "PROCESSING",
                  status: "EXECUTING"
                });
              }
              break;
        ...
        // Catch edge case terminal states if task crashes at engine level without sending MISSION_COMPLETED
        if (taskState === "SUCCESS" && eventData.event !== "MISSION_COMPLETED") {
          setceleryId(null);
          setStatus('idle');
        } else if (taskState === "FAILURE" || taskState === "REVOKED") {
          console.error("Critical System Warning: Worker execution environment aborted task process.");
          setceleryId(null);
          setStatus('idle');
          setIsPaused(true);
        }
        ...
        console.error("Fault encountered during active background telemetry polling update:", error);
      }
    };

    // Instantiate evaluation worker loops at 1500ms intervals
    const trackerInterval = setInterval(pollWorkerTelemetry, 1500);
    return () => {
      isMounted = false;
      clearInterval(trackerInterval);
    };
  }, [celeryId, isPaused, status, steps, onMetricsUpdate, onCompleteExecution, activeMissionId]);
