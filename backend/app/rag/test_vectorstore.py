...
from chunker import rag_chunker
from vector_store import rag_vector_store

long_text = """In this article, I'd like to share 30 “one-sentence stories” about building better habits. (They are not all exactly one sentence, but they are very short.)
...
chunks = rag_chunker.create_chunks(long_text)

print(f"Indexing {len(chunks)} chunks...")
embeddings = rag_embedder.generate_embeddings(chunks)
start_id = rag_vector_store.add_to_index(embeddings=embeddings)

query = "How can I change my environment to stop bad habits?"
print(f"\nUser Query: {query}")

query_embedding = rag_embedder.generate_embeddings(query)
distances, indices = rag_vector_store.search(query_embedding)

best_chunk_index = indices[0]
print(f"Top Match found at Index: {best_chunk_index}")
