...
USER_ID = 3 #hardcoded for testing, do not need to find a user for mere testing in the backend

article_text = """In this article, I'd like to share 30 “one-sentence stories” about building better habits. (They are not all exactly one sentence, but they are very short.)
...
try:
    #Ingest(Input)
    print("Ingesting the text input...")
    result = ingest_text(db = db, title="Testing", raw_text=article_text, user_id=USER_ID)
    print(f"Successful! Created Doc ID: {result['document_id']} of chunk length: {result['chunks_count']}")

    #Retrieve
    query = "Why is environment design important for habits?"
    print(f"\nTesting Retrieval for: {query}")

    context = rag_retriever.retrieve_context(query, db)
