...
class MissionExecutor:
    ...
    async def perform_task(self, tool, step_data):
        try:
            if tool == "web_search":
                ...
                tool_output = "Error : Unidentifiable tool"
    ...
    async def run_step_with_retries(self, step: dict, strategy: str, max_retries=2) -> str:
        step_id = step.get('backend_step_id','UNKNOWN')
        ...
        for attempt in range(max_retries + 1):
            try:
                payload = {"instruction": desc, "context": self.step_context}
