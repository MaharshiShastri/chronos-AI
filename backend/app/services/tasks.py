...
def validate_and_correct_steps(steps: list, total_budget: int) -> list:
...
def execute_mission_task(self, mission_id, total_budget, manifest):
    ...
    return loop.run_until_complete(async_execute_mission(self, mission_id, total_budget, manifest))

async def async_execute_mission(task, mission_id: int, total_budget: int, steps: list):
    # Initialize the stateful executor
    executor = MissionExecutor(mission_id, total_budget)
    corrected_steps = validate_and_correct_steps(steps, total_budget)
    ...
    for i, step in enumerate(corrected_steps):
        ...
        if val['status'] == "AMBIGUOUS" and strategy != "EMERGENCY":
            ...
            task.update_state(state='PROGRESS', meta={
                ...
                'step_id': step['backend_step_id'],
                'reason': val.get('reason', 'Input Verification Failed')
            })
