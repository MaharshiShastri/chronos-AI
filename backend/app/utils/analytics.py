...
# Dynamically resolve paths relative to your project root
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

class ChronosAnalytics:
    def __init__(self, failure_log: str, metrics_log: str):
        self.failure_log = failure_log
        self.metrics_log = metrics_log

    def _parse_indented_logs(self, file_path: str):
    ...
    def export_kpi_summary(self, output_path: str):
        stats = self.get_system_kpis()
        with open(output_path, "w") as f:
            json.dump(stats, f, indent=4)
        return stats

# Initialized using robust dynamic paths instead of hardcoded Windows drives
analytics_engine = ChronosAnalytics(
    os.path.join(BASE_DIR, "failure.log"), 
    os.path.join(BASE_DIR, "retrieval_metrics.log")
)